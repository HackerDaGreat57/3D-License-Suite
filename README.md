...
In this README, names of companies and products are slightly altered to avoid legal trouble. We don't mean to make fun of y'all; it's just that I don't feel it's a smart idea to take real names.
...
### Hardware (3dHL)
You have full ownership of the physical hardware you purchased and you can use the hardware to its full potential without having to pay more, provide details, be related to someone, or login to a service. You can also obtain the associated CAD/design files without having to pay, provide details, be related to someone, or login to a service. You may modify it as you wish, but if you are publicly distributing physical versions of your modifications, then you need to also distribute the design files of your modifications. And all modifications (except physical versions) which you distribute must be available to users without them having to pay, provide details, be related to someone, or login to a service. It is mandatory to give credit to the people who made the original version of the design; we don't like plagiarism 'round here.

If you wish to use and/or include design files or physical versions of the hardware in other hardware, the other hardware must pass the following criteria:

1. Anyone must be able to obtain the other hardware's CAD/design files without having to pay, provide details, be related to someone, or login to a service. It is OK if people have to pay, provide details, and/or log in to services to get physical versions.
2. The other hardware should continue working to its full potential for as long as it physically can under the usage conditions. (The hardware should not be designed to fail.)

#### Use Cases
A renowned motherboard company *Aysus* would like to use a 3dHL-licensed implementation of PCIx 5 in its products. They must either relicense the affected products under the 3dHL or use a custom implementation.
## But why?
### Software
...
### Hardware
The fire burning here is way less intense than the one in software. In fact, there's no fire at all. It's just that we'd like to see hardware companies share their work so that tinkerers, inventors, and employees alike can collaborate in order to produce the best product. Just like how both hobbyists and employees work on VS Code, in order to create and make better the Swiss Army knife of IDEs.

When hardware is under this license, you can always be sure that you have the [right to repair](https://en.wikipedia.org/wiki/Right_to_repair#Definition). It also allows you to tinker and possibly make the hardware better, which in turn should increase sales of the company/group that created it (assuming they implement your changes into the original); it's a win-win situation.
